...
class CourseTest(unittest.TestCase):
    ...
    def test_get_course_sections(self):
        with patch('requests.Session') as mock:
            ...
            self.assertEqual(cs_course.course_number, '0007')
            self.assertEqual(cs_course.term, '2194')
            self.assertEqual(cs_course.subject_code, 'CS')

            # self.assertEqual(cs_course[0], cs_course.sections[0])
            for section in cs_course.sections:
                self.assertEqual(cs_course.term, section.term)
            # Confirm this is why the above test was written; to make sure terms match of Course and each internal Section

            # self.assertEqual(len(cs_course[0:2]), 2)
            # Not sure of purpose behind this test

            # self.assertEqual(cs_course[0].course_number, cs_course.number)
            # Not sure of purpose behind this test; an individual Section does not contain course number

            # self.assertEqual(cs_course[0].course_title, cs_course.title)
            # Not sure of purpose behind this test; an individual Section does not contain course title

            self.assertEqual(str(cs_course), 'Course(2194, CS, 0007)')
            # Created new __str__ method in Course to accomodate this format

            # self.assertEqual(repr(cs_course), json.dumps(cs_course.to_dict()))
            # print(repr(cs_course))
            # Custom repr method in Course should cover what this test intends
