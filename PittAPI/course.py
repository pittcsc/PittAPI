...
SECTION_DETAIL_URL = (
    "https://psmobile.pitt.edu/app/catalog/classsection/UPITT/{term}/{section_number}"
)
    ...
    def parse_webpage(self, resp: requests.Response) -> None:
        ...
        for child in classes:
            if "href" in child.attrs:
                ...
                self._courses[course_number] = PittCourse(
                    parent=self, course_number=course_number, course_title=course_title
                )
                course = self._courses[course_number]

    def to_dict(self, extra_details: bool = False) -> Dict[str, List[Dict[str, Any]]]:
        return {
            k: v.to_dict(extra_details=extra_details) for k, v in self._courses.items()
        }

    def __str__(self):
        return "PittSubject({term}, {subject})".format(
            term=self.term, subject=self.subject, num=len(self._courses)
        )
...
class PittCourse:
    ...
    def to_dict(self, extra_details: bool = False) -> List[Dict[str, Any]]:
        return [
            section.to_dict(extra_details=extra_details) for section in self.sections
        ]

    def __str__(self) -> str:
        return "PittCourse({term}, {subject}, {number})".format(
            term=self.term, subject=self.subject, number=self.number
        )
...
class PittSection:
    def __init__(
        self,
        parent: Union["PittSubject", None],
        course: Union["PittCourse", None],
        class_section_url: str,
        class_data: Union[List[str], None],
        *,
    ...
    def __set_properties(self, data):
        ...
        if days_times != "TBA":
            ...
            self.days = [
                self.days[i * 2 : (i * 2) + 2] for i in range(len(self.days) // 2)
            ]
    ...
    def extra_details(self) -> Dict[str, Any]:
        ...
        if "Enrollment Requirements" in data[5].text:
            self._extra["preq"] = self.__extract_data_past_colon(
                self.__extract_data_from_div_section(data[5])
            )
            if "Class Attributes" in data[6].text:
                self._extra["class_attributes"] = self.__extract_data_from_div_section(
                    data[6]
                ).split("\n")
...
def get_course_sections(
    ...
    response = session.post(CLASS_SEARCH_API_URL, data=payload)
    container = PittCourse(
        parent=None, course_number=course, term=term, subject=subject
    )
    container.parse_webpage(response)
    return container
...
def get_section_details(
    ...
    response = session.post(CLASS_SEARCH_API_URL, data=payload)
    container = PittSection(
        parent=None, course=None, class_section_url="", class_data=None, term=term
    )
    container.parse_webpage(response)
    return container
